...
-- Nutzt AssetCatalog als Quelle für Brainrot-Definitionen (Price/MiningPower/Income/Rarity)
-- Fügt ProximityPrompt zum Kaufen hinzu (Client feuert BrainrotPickup mit Modelname)
-- Exposes _G.spawnBrainrotModel für PersistenceInit
...
local function getPlotPosition(plot)
	...
	warn("⚠️ Plot " .. plot.Name .. " hat keinen BrainrotSpawn! Fallback wird verwendet.")
...
-- Expose für PersistenceInit
_G.spawnBrainrotModel = spawnBrainrotModel

print("🔍 RollSystem ready")
...
for _, plot in ipairs(plotsFolder:GetChildren()) do
	...
	cd.MouseClick:Connect(function(player)
		...
		if not pos then
			warn("RollSystem: Keine gültige Spawn-Position für Plot " .. plot.Name)
			return
		end
		...
		if spawned then
			print("🎲 " .. player.Name .. " rolled " .. tostring(name) .. " [" .. tostring(def.Rarity or "Common") .. "] Power:" .. tostring(def.MiningPower))
		else
			warn("RollSystem: Spawn fehlgeschlagen für " .. tostring(name))
		end
	end)
end

print("✅ RollSystem live")
